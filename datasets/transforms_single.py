...
def crop(image, depth, target, region):
...
def pad(image, depth, target, padding):
    # assumes that we only pad on the bottom right corners
    padded_image = F.pad(image, (0, 0, padding[0], padding[1]))
    padded_depth = F.pad(depth, (0, 0, padding[0], padding[1])) if depth is not None else None
    if target is None:
...
class ToTensor(object):
    def __call__(self, img, depth, target):
        ...
        return F.to_tensor(img), depth, target
...
class Normalize(object):
    ...
    def __call__(self, image, depth, target=None):
        # Apply the first 3 mean and std values to image
        image = F.normalize(image, mean=self.mean[:3], std=self.std[:3])
        
        # Applythe last mean and std values to depth
        if self.has_depth_val and depth is not None:
            depth = F.normalize(depth, mean=self.mean[3:], std=self.std[3:])
        elif self.has_depth_val == False and depth is not None: 
