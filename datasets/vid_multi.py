...
class CocoDetection(TvCocoDetection):
    def __init__(self, img_folder, depth_folder, ann_file, transforms, return_masks, interval1, interval2, num_ref_frames= 3,
        is_train = True,  filter_key_img=True,  cache_mode=False, local_rank=0, local_size=1, use_depth=False):
    ...
    def __getitem__(self, idx):
        ...
        if video_id == -1: # imgnet_det
            ...
            if self.is_train: # Train
                # --- Method 2: Only select the nearby 4 reference frames
                interval = self.num_ref_frames # 
                ...
                # --- Method 1: Randomly select reference frames in an interval of (left+2, right+2)
                ...
                while len(ref_img_ids) < self.num_ref_frames:
                    print("Reference range", ref_img_ids)
            ...
            for ref_img_id in ref_img_ids:
                ...
                ref_depth = self.get_depth(ref_depth_path)
                imgs.append(ref_img)
                depths.append(ref_depth)

        if self._transforms is not None:
            imgs, depths, target = self._transforms(imgs, depths, target)
        
        final_imgs = []
        if self.use_depth:   
            for img, depth in zip(imgs, depths):
                assert img.shape[0] == 3, "Image should be 3 channel RGB"
                assert depth.shape[0] == 1, "Depth should be 1 channel"
                combined_imgs = torch.cat([img, depth], dim=0)
                final_imgs.append(combined_imgs) 
            return  torch.cat(final_imgs, dim=0),  target
        else:
...
def make_coco_transforms(image_set, args):
    # Normalize the image to the ImageNet mean and std, dformer settings for depth channel
    if args.use_depth:
        normalize = T.Compose([
            T.ToTensor(),
    ...
    # Only augmentations used for training
...
def build(image_set, args):
    ...
    # TODO: Change the dataset accordingly based on the custom datasets 
    PATHS = {
        "train_joint": [(root, root, root / 'coco' /  'annotations_multi'  / 'train.json')],
        "val": [(root, root, root / 'coco' /  'annotations_multi'  / 'train.json')],
    }
    
    datasets = []
    for (img_folder, depth_folder, ann_file) in PATHS[image_set]:
        dataset = CocoDetection(img_folder, depth_folder, ann_file, transforms=make_coco_transforms(image_set, args), is_train =(not args.eval), interval1=args.interval1,
                                interval2=args.interval2, num_ref_frames = args.num_ref_frames, return_masks=args.masks, cache_mode=args.cache_mode, 
                                local_rank=get_local_rank(), local_size=get_local_size(), use_depth=args.use_depth)
