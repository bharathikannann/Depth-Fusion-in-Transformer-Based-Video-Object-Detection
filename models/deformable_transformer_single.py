...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 use_depth=False, depth_type="Baseline_rgb", dpth_feature_levels=1, dpth_n_points=4,
                 ):
        super().__init__()
        ...
        if "encoder_cf" in self.depth_type:
            ...
            # Encoder Layers if not using encoder cross fusion
            encoder_layer = DeformableTransformerEncoderLayer(d_model, dim_feedforward,
                                                            dropout, activation,
                                                            num_feature_levels, nhead, enc_n_points)
    ...
    def forward(self, srcs, masks, pos_embeds, depth_srcs, depth_masks, depth_pos_embeds, query_embed=None, rgbd_query=[]):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            ...
            if len(rgbd_query) > 0:
                rgbd_src = rgbd_query[lvl].flatten(2).transpose(1, 2)
                rgbd_flatten.append(rgbd_src)
            mask = mask.flatten(1)
        ...
        if len(rgbd_query) > 0:
            rgbd_flatten = torch.cat(rgbd_flatten, 1)
        ...
        if "latefusion" in self.depth_type and self.use_depth:
            ...
            # print("Time for depth encoder", time.time() - let)
            src_flatten = src_flatten + fused_src_flatten
            
            # For research purposes: Second layer of LateFusion, can be ignored
            # depth_src_flatten_resized = F.interpolate(depth_src_flatten.transpose(1, 2), size=src_flatten.shape[1], mode='linear', align_corners=False).transpose(1, 2)
            # src_flatten = src_flatten + depth_src_flatten_resized
            # second stage of latefusion
            # if self.late_fusion_layers > 1:
            #     fused_src_flatten2 = self.depth_encoder_layer2(src_flatten, lvl_pos_embed_flatten, depth_lvl_pos_embed_flatten, spatial_shapes, rgb_reference_points, depth_reference_points, depth_src_flatten, depth_spatial_shapes, depth_lvl_start_index, mask_flatten, depth_mask_flatten)
            #     src_flatten = src_flatten + fused_src_flatten2 
            # if "2way" in self.depth_type:
            #     # Get reference points for RGB
            #     rgb_reference_points = self.get_reference_points(depth_spatial_shapes, depth_valid_ratios, device=depth_src_flatten.device)
            #     # Deformable cross attention between Depth and RGB (RGB single scale or multi scale, Depth single scale)
            #     fused_depth_src_flatten = self.rgb_encoder_layer(depth_src_flatten, depth_lvl_pos_embed_flatten, depth_spatial_shapes, rgb_reference_points, src_flatten, spatial_shapes, level_start_index, depth_mask_flatten, mask_flatten)
            #     if self.residual_fusion:
            #         depth_src_flatten = depth_src_flatten + fused_depth_src_flatten
            #     else:
            #         depth_src_flatten = fused_depth_src_flatten
                    
            #     if "concat" in self.depth_type:
            #         rgb_src_flatten = src_flatten + lvl_pos_embed_flatten
            #         depth_src_flatten = depth_src_flatten + depth_lvl_pos_embed_flatten
            #         if rgb_src_flatten.shape[-2] != depth_src_flatten.shape[-2]:
            #             # Interpolate the depth features to the RGB scale
            #             depth_src_flatten = F.interpolate(depth_src_flatten.unsqueeze(0),   size=(rgb_src_flatten.shape[1], depth_src_flatten.shape[2]), mode='bilinear', align_corners=False).squeeze(0)
            #         rgbd_src_flatten = torch.cat([rgb_src_flatten, depth_src_flatten], 2)
            #         rgbd_flatten = self.rgb_d_concat(rgbd_src_flatten)           
...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None, rgbd_src=None):
        # self attention
        src2 = None
        if rgbd_src is not None:
            src2 = self.self_attn(rgbd_src, reference_points, src, spatial_shapes, level_start_index, padding_mask)
        else:
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None, rgbd_src=None):
        output = src
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=src.device)
        for _, layer in enumerate(self.layers):
            output = layer(output, pos, reference_points, spatial_shapes, level_start_index, padding_mask, rgbd_src=rgbd_src)
