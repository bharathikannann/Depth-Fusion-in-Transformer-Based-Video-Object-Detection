# Single frame Deformable DETR model
...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, depth_backbone, transformer, num_classes, num_queries,
                 num_feature_levels, aux_loss=True, with_box_refine=False, two_stage=False,
                 use_depth=False, depth_type=''):
        """Initializes the model.

        Parameters:
            backbone: torch module of the backbone to be used.
            depth_backbone: torch module of the depth backbone to be used.
            ...
            depth_type: type of depth processing.
        """
        super().__init__()
        # Basic settings
        ...
        # Determine if crossfusion features concatenation is used, For Research Scripts
        ...
        # Set hidden dimension from transformer
        hidden_dim = transformer.d_model

        # Initialize classification and bounding box regression heads
        self.class_embed = nn.Linear(hidden_dim, num_classes)
        self.bbox_embed = MLP(hidden_dim, hidden_dim, 4, 3)

        # Define the number of channels for different backbones
        self.num_channels = {
        ...
        # Initialize query embeddings if not using two-stage
        if not two_stage:
        ...
        if num_feature_levels > 1:
            ...
            for idx in range(num_backbone_outs):
                in_channels = self.num_channels[backbone.name][idx]
                input_proj_list.append(nn.Sequential(
                    nn.Conv2d(in_channels, hidden_dim, kernel_size=1),
                    nn.GroupNorm(32, hidden_dim),
                ))
            in_channels = self.num_channels[backbone.name][num_backbone_outs - 1]
            ...
            in_channels = self.num_channels[backbone.name][-1]
        ...
        # Initialize depth projection layers if using depth and crossfusion features concatenation
        ...
        # Initialize depth projection layers for specific depth types
        if (('dformer' in self.depth_type and 'latefusion' in self.depth_type) or
        ...
        # Initialize weights for input projections
        for proj in self.input_proj:
            nn.init.xavier_uniform_(proj[0].weight, gain=1)
            nn.init.constant_(proj[0].bias, 0)

        # Initialize weights for depth projections if used
        if self.use_depth and self.crossfusion_features_concat:
        ...
        # Configure two-stage and with_box_refine settings
        num_pred = (transformer.decoder.num_layers + 1) if two_stage else transformer.decoder.num_layers
        if with_box_refine:
            self.class_embed = _get_clones(self.class_embed, num_pred)
            self.bbox_embed = _get_clones(self.bbox_embed, num_pred)
            nn.init.constant_(self.bbox_embed[0].layers[-1].bias.data[2:], -2.0)
            # Assign bbox_embed to transformer decoder for iterative refinement
        ...
        if two_stage:
            # Assign class_embed to transformer decoder for two-stage
    ...
    def forward(self, samples: NestedTensor):
        ...
        # Ensure samples are in NestedTensor format
        ...
        # Split RGB and Depth tensors if applicable
        ...
        # Cross Fusion based on depth_type
        ...
        # Handle additional feature levels if required
        ...
        # Project Depth features if applicable
        depth_srcs = []
        depth_masks = []
        ...
        # Handle crossfusion features concatenation, currently not supported 
        rgbd_query = []
        if self.crossfusion_features_concat:
            for l, feat in enumerate(depth_features):
                src, mask = feat.decompose()
            ...
            for i, (rgb, rgb_pos, depth, depth_pos_item) in enumerate(zip(srcs, pos, depth_srcs, depth_pos)):
                # Ensure spatial dimensions match
                if rgb.shape[2:] != depth.shape[2:]:
                    depth_srcs[i] = F.interpolate(depth, size=rgb.shape[2:], mode='bilinear', align_corners=False)
                    depth_pos[i] = F.interpolate(depth_pos_item, size=rgb.shape[2:], mode='bilinear', align_corners=False)
        ...
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact = self.transformer(srcs, masks, pos, depth_srcs, depth_masks, depth_pos, query_embeds, rgbd_query)
        outputs_classes = []
        outputs_coords = []
        ...
        # For testing the Integrated gardients methods, uncomment and test the model
        # out = outputs_classes[-1][0][:,1]
        # return out 
        ...
        # t1 = time.time()
        # print(f"Time taken for forward pass: {t1 - t}")
        return out
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        if out_logits.shape[-1] == 3: # For binary classification, remove the last class
            topk_values, topk_indexes = torch.topk(prob[:, :, :-1].reshape(out_logits.shape[0], -1), 100, dim=1)
