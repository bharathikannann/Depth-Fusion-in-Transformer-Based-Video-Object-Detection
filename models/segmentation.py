...
# Modified sigmoid focal loss for the dataset, change the alpha values for the custom dataset.
def modified_sigmoid_focal_loss(inputs, targets, num_boxes, alpha: float = 0.25, gamma: float = 4):
    """
    ...
    # Set the last column of targets to 1 as it represents no object
    targets[..., 2] = (targets[..., 1] != 1).long()
    ce_loss = F.binary_cross_entropy_with_logits(inputs, targets, reduction="none")
    p_t = prob * targets + (1 - prob) * (1 - targets)
    loss = ce_loss * ((1 - p_t) ** gamma)

    # Alpha balancing factor for the focal loss
    alpha_values = torch.tensor([0.0, 1.0, 0.001]).to(targets.device)
    alpha_t = alpha_values[None, None, :]  # Add two dimensions to match the targets shape
