...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, depth_srcs, depth_masks, depth_pos_embeds, query_embed=None, class_embed = None, rgbd_query = []):
        ...
        topk_values, topk_indexes =  torch.topk(prob[:,:,:-1].reshape(ref_hs_logits.shape[0], -1), 80 * self.num_ref_frames, dim=1)
        topk_indexes = topk_indexes // (ref_hs_logits.shape[2] - 1)
        ...
        topk_values, topk_indexes =  torch.topk(prob[:,:,:-1].reshape(ref_hs_logits.shape[0], -1), 50 * self.num_ref_frames, dim=1)
        topk_indexes = topk_indexes // (ref_hs_logits.shape[2] - 1)
        ...
        topk_values, topk_indexes =  torch.topk(prob[:,:,:-1].reshape(ref_hs_logits.shape[0], -1), 30 * self.num_ref_frames, dim=1)
        topk_indexes = topk_indexes // (ref_hs_logits.shape[2] - 1)
