...
# ------------------------------------------------------------------------
# TransVOD does not support Depth Fusion methods yet
# ------------------------------------------------------------------------
...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, depth_backbone, transformer, num_classes, num_queries, num_feature_levels, 
        ...
        self.crossfusion_features_concat = True if 'crossfusion_2way_concat' in depth_type else False
        ...
        self.num_channels = {"resnet18": [64, 128, 256, 512], "resnet50": [64, 512, 1024, 2048], 'dformer':[32, 64, 128]}
        ...
        if num_feature_levels > 1:
            ...
            for _ in range(num_backbone_outs):
                in_channels = self.num_channels[backbone.name][_]
        ...
        if 'dformer' in self.depth_type and 'latefusion' in self.depth_type or "encoder_cf" in self.depth_type:
            self.input_proj_depth = nn.ModuleList([
                    nn.Sequential(
        ...
        if self.use_depth and self.crossfusion_features_concat:
            for proj in self.input_proj_depth and self.crossfusion_features_concat:
                nn.init.xavier_uniform_(proj[0].weight, gain=1)
                nn.init.constant_(proj[0].bias, 0)
    ...
    def forward(self, samples: NestedTensor):
        ...
        # if self.use_depth and "latefusion" in self.depth_type:
        ...
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, final_hs, final_references_out = self.transformer(srcs, masks, pos, depth_srcs, depth_masks, depth_pos, query_embeds, self.class_embed[-1], rgbd_query)
